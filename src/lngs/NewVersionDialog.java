...
public class NewVersionDialog extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jTextPane_Changes.setText("o Note: There are rumors that the company I work for is switching from Lotus Notes to Outlook. (Yay for me!) The switch may be months or years away, but I want to start looking for volunteers to maintain LNGS. If you are interested, email deanhill1971@gmail.com.\no Fix: If the proxy server or port were changed, LNGS would not notice the change until being restarted. Now the configuration change is picked up immediately.\no Fix: Fixed a rare problem where the Synchronize button would stay disabled if a proxy server wasn't defined.\no Fix: The way LNGS determines if a calendar entry is repeating has been changed. This fixes a problem for a user.\no Fix: The way LNGS retrieves start/end dates for a repeating calendar entry has been changed. This fixes a problem for a user.\no Fix: In lngsync.vbs, added some more Registry search paths to find Lotus Notes values and made improvements for Lotus Notes v9. Thanks to JPonte and Peter Rosenberg.\no Fix: There were some situations where LNGS would delete entries created in GCal (instead of leaving them alone). Thanks to Duythoven for reporting the problem.\no Enhancement: Each Google Calendar can have 0 to 5 default reminders/notifications which are used when a new calendar entry is created. Previously LNGS would never use these default GCal reminders. Now, if \"Alarms Become Google Reminders\" is unchecked, then the GCal default reminders will be used by LNGS. When the box is checked, the Lotus Notes alarms are used instead of the GCal defaults.\no Enhancement: In the Help File, added a Troubleshooting decription for how to resolve an SSL certificate problem.\no Quote: Time you enjoy wasting, was not wasted. - John Lennon");
